eta_update_ftn <- function(eta_j_input, k_input){
  ...
  return(list(eta_j_output=eta_j_output, prob=prob))
...
plot(x,y(2,x),type="l", ylab="eta",main="eta target function depending on beta")
lines(x,y(3,x),col="red")
lines(x,y(4,x),col="blue")
legend(4,0.8, legend=c("2","3","4"), col=c("black", "red","blue"), lwd=1, cex=0.8)


plot(x,y(2,x),type="l", ylab="f(x)",main=expression(f(x)==frac(e^-{2*x},1+x)), ylim=c(0,2),xlim=c(0,2))
lines(x,3*y(2,x),col="red")
legend(1.2,1.5, legend=c("f(x)","2f(x)"), col=c("black", "red"), lwd=1, cex=0.8)
...
# weight check
wgt_mat <- matrix(0, rep, 4)
eta_temp <- numeric(rep)
eta_temp[1] <- 0.5
for(i in 2:rep){
  eta_temp[i] <- eta_update_ftn(eta_temp[i-1],k[1])$eta_j_output
  wgt_mat[i,] <- eta_update_ftn(eta_temp[i-1],k[1])$prob
}

apply(wgt_mat,2,mean)

# negative log
xx <- seq(0,5,0.01)
yy <- log(xx+0.5)
plot(xx,yy,type="l",xlab="x",ylab="h(x)")
